	 ...
	 public function getMenuCustomerLink($lang_id = NULL,$setting = array()) {
        ...
        if ($showhome) {
			$act = '';
			 ...
			 // echo $url; die;
            $id = "_home";
            $_menuLink .= '<div id="pt_ver_menu' . $id . '" class="pt_ver_menu' . $act . '">';
            $_menuLink .= '<div class="parentMenu">';
            $_menuLink .= '<a href="'.$url.'">';
            $_menuLink .= '<span>' . $this->language->get("text_home"). '</span>';
            $_menuLink .= '</a>';
            $_menuLink .= '</div>';
            $_menuLink .= '</div>';
        }
		...
		foreach ($menu_items as $item)
		{
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				case 'CAT':
					$item1 = $item1+ 1; 
					$_menuLink .= $this->drawCustomMenuItem($id, 0, false, $item1, $lang_id,$setting);
					break;
				case 'LINK': 
                    ...
                    if($link) {
                        ...
                        $_menuLink .= '<div id ="pt_ver_menu_link" class ="pt_ver_menu '. $over_class .'"><div class="parentMenu" ><a href="'.($link['link']).'"><span>'.$link['title'].'</span></a></div></div>'.PHP_EOL;
                    }
				case 'CMS': 
                    ...
                    if(isset($cms['information_id'])) {
                        ...
                        $_menuLink .= '<div id ="pt_ver_menu_cms' . $cms['information_id'] . '" class ="pt_ver_menu '. $over_class .'"><div class="parentMenu" ><a href="'.$cms_link.'"><span>'.$cms['title'].'</span></a></div></div>'.PHP_EOL;
                    }
			}
            $count++;        
		}
        ...
        $this->load->language('extension/module/vermegamenu');
         ...
         foreach ($blockCustomer as $bc) {
             if($bc['status']) {
                 $_menuLink .= $this->drawCustomMenuBlock($bc['identify'], $bc, $setting);
             }
         }

        if($setting['is_over']) {
            $_menuLink .= '<div class="more-wrap pt_ver_menu"><div class="parentMenu"><a href="javascript:void(0);" class="more-ver-menu"><span>'.$this->language->get('text_more_vertical_menu').'</span></a></div></div>';
            $_menuLink .= '<div class="close-wrap pt_ver_menu over-menu"><div class="parentMenu"><a href="javascript:void(0);" class="close-ver-menu"><span>'.$this->language->get('text_close_vertical_menu').'</span></a></div></div>';
        }
...
//        $html[] = '</a>';
        ...
        if ($drawPopup) {
            ...
            if ($blockHtml) {
                $html[] = '<div class="block2" id="v_block2' . $id . '">';
                $html[] = $blockHtml;
                $html[] = '</div>';
            }
            $html[] = '</div>';
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				case 'LINK': 
					...
					if($link) {
						$link = $link[0];
						//echo "<pre>"; print_r($cms); echo "</pre>";
						$_menuLink .= '<li><a href="' . ($link['link']) . '"><span>' . $link['title'] . '</span></a></li>' . PHP_EOL;
					}
				case 'CMS': 
					$this->load->model('catalog/information');
					$cms = $this->model_catalog_information->getInformation($id);
                    if(isset($cms['information_id'])) {
    					//echo "<pre>"; print_r($cms); echo "</pre>"; 
    					$cms_link = $this->url->link('information/information', 'information_id=' . $cms['information_id']);
    					$_menuLink .= '<li><a href="'.$cms_link.'"><span>'.$cms['title'].'</span></a></li>'.PHP_EOL;
                    }
			}
