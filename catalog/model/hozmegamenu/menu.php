...
class ModelHozmegamenuMenu extends Model {
	 ...
	 public function getMenuCustomerLink($lang_id = NULL,$setting = array()) {
        ...
        if ($showhome) {
		   ...
		   if(isset($this->request->get['route'] )) {
            ...
            $id = "_home";
            $_menuLink .= '<div id="pt_menu' . $id . '" class="pt_menu' . $act . '">';
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'LINK': 
					...
					if($link) {
						...
						$_menuLink .= '<div id ="pt_menu_link' . $id . '" class ="pt_menu"><div class="parentMenu" ><a href="' . ($link['link']) . '"><span>' . $link['title'] . '</span></a></div></div>' . PHP_EOL;
					}
				case 'CMS': 
                    ...
                    if(isset($cms['information_id'])) {
    					...
    					$_menuLink .= '<div id ="pt_menu_cms" class ="pt_menu"><div class="parentMenu" ><a href="'.$cms_link.'"><span>'.$cms['title'].'</span></a></div></div>'.PHP_EOL;
                    }
			}
		...
		foreach ($blockCustomer as $bc) {
			...
			if($bc['status'] && $flag) {
				$_menuLink .= $this->drawCustomMenuBlock($bc['identify'], $bc);
			}
        }
	...
	public function getMenuCustomerLinkMobile($lang_id = NULL,$setting = array()) {
		...
		$item1=0;
		$lang_id = (int)$this->config->get('config_language_id');
		$_menuLink = '<ul id="ma-mobilemenu" class="mobilemenu originalmenu nav-collapse collapse">';
		...
		foreach ($menu_items as $item)
		{
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				case 'CAT':
					...
					if($curr_cate) {
						$item1 = $item1+ 1; 
						$_menuLink .= $this->drawCustomMenuItemMobile($id, 0, false, $item1, $lang_id,$setting);
...
//        $html[] = '</a>';
	...
	public function drawCustomMenuItem($category, $level = 0, $last = false, $item, $lang_id,$setting) {
        ...
        if ($drawPopup) {
            $html[] = '<div id="pt_menu' . $id . '" class="pt_menu_has_child pt_menu' . $active . ' nav-' . $item . '">';
        } else {
